...
function App() {
    ...
    const gqlQueryString = (queryBuilder && targetObject) ? queryBuilder.makeFullQuery(parentSpecs, targetObject) : null
    ...
    if (targetData && Array.isArray(targetData)) {
        return (<>{targetData.map(row => <div>{Object.values(row)}</div>)}</>)  // TODO: add key
    }
